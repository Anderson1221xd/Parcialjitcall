...
export class ChatPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.chatService
      .getOrCreateConversation(this.currentUserId, this.otherUserId)
      .then((chatId) => {
        this.chatId = chatId;
        this.messages$ = this.chatService.watchMessages(chatId);
      });
  }
  ...
  sendMessage() {
    ...
    this.chatService.postMessage(this.chatId, message).then(() => {
  ...
  sendOtherMessage(file: any, type: typefile) {
    ...
    this.chatService.postMessage(this.chatId, message);
  }
  ...
  async selectFile() {
    ...
    if (file) {
      ...
      this.bucketSrv
        .storeFile(file.blob || new Blob(), `${this.currentUserId}`)
        .then((url) => {
  ...
  sendPicture() {
    this.cameraSrv
      ...
      .then((blob) => {
        ...
        this.chatService.postMessage(this.chatId, message);
        this.bucketSrv
          .storeImage(blob, `${this.currentUserId}`)
          .then((url) => {
  ...
  async toggleRecording() {
    try {
      if (!this.isRecording) {
        ...
        const { recordDataBase64, mimeType } =
          await this.voiceSrv.stopRecording();
        ...
        if (blob) {
          this.bucketSrv
            .storeAudio(blob, `${this.currentUserId} ${Date.now()}.aac`)
            .then((url) => {
